...
$query_turns = "SELECT `turn_id`, `game`, `league`, `season`, `week`, `uploadID` FROM `g_turnsummary`";
...
foreach ($data_turn1 as $shirt_num => $player1) {
    if (isset($data_turn2[$shirt_num])) {
        $player2 = $data_turn2[$shirt_num];
...
$table_rows .= '<tr>';
$table_rows .= '<td>' . htmlspecialchars($player1['team_name']) . '</td>';
$table_rows .= '<td>' . htmlspecialchars($player1['m_sh']) . '</td>';
$table_rows .= '<td>' . htmlspecialchars($player1['m_cname'] . ' ' . $player1['m_sname']) . '</td>';
$table_rows .= '<td>' . htmlspecialchars($player1['m_type']) . '</td>';
$table_rows .= '<td>' . htmlspecialchars($player1['m_pos']) . '</td>';
...
$table_rows .= '<td' . 
    ($player1['m_val'] > 99 ? ' class="w3-red"' : ($player1['m_val'] >= 70 && $player1['m_val'] <= 99 ? ' class="w3-yellow"' : '')) . 
...
echo '        <h3 class="w3-text-blue">Summary of Changes</h3>';
